...
<template>
  <div class="lesson-header font-large font-light text-uppercase" data-type="issueCredentialTitle">
    {{ t('steps.user.issue.title') }}
  </div>
  <div ref="lessonTextContainer" class="lesson-text" data-type="lessonTextContainer">
    ...
    <WarningAlert
      ...
      :text="t('alert_messages.doConnectionTutorial')"
      data-type="warningAlert"
    />

    <div
      @click="startTutorialIssueCredential"
      class="tutorial-button btn button-outline-primary mt-4 d-flex justify-content-between"
      data-type="startIssueCredentialButton"
    >
      <span></span>
      {{ t('tutorial.issue_credential.title') }}
      <font-awesome-icon
        v-if="demoTutorialStore.getIssueTutorialFinished"
        class="font-large"
        icon="circle-check"
      />
    </div>
