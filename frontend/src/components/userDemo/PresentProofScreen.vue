...
<template>
  <div class="lesson-header font-large font-light text-uppercase" data-type="presentTitle">
    {{ t('steps.user.present.title') }}
  </div>
  <div ref="lessonTextContainer" class="lesson-text" data-type="lessonTextContainer">
    ...
    <p class="font-medium font-light">
      <i>{{ t('steps.user.present.paragraph3') }}</i>
    </p>
    <WarningAlert
      v-if="state.showWarning"
      :text="t('alert_messages.doIssueTutorial')"
      data-type="warningAlert"
    />
    <div
      @click="startTutorialPresentProof"
      class="tutorial-button btn button-outline-primary p-3 mt-4 d-flex justify-content-between"
      data-type="startPresentProofTutorial"
    >
      <span></span>
      {{ t('tutorial.present_proof.title') }}
      <font-awesome-icon
        v-if="demoTutorialStore.getPresentTutorialFinished"
        class="font-large"
        icon="circle-check"
      />
    </div>
