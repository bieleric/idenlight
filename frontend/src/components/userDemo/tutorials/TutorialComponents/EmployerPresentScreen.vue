    ...
    <div class="accordion" id="accordionExample">
        ...
        <h2 class="accordion-header" id="headingOne">
          ...
          >
            <font-awesome-icon icon="circle-check" />&nbsp;{{
              t('tutorial.employer_website.cover_letter')
            }}
          </button>
        </h2>
        <div
      ...
      <div class="accordion-item">
        ...
        >
          ...
          >
            <div>
              <a :href="state.invitation_url">{{
                t('tutorial.employer_website.create_connection')
              }}</a>
            </div>
            <QrcodeVue
              class="mx-auto mt-3"
              :value="webhookStore.getInvitationURL"
              :size="state.size"
              level="H"
              data-type="presentQRCode"
            />
          </div>
          <div v-else class="d-flex justify-content-center flex-column">
            <div @click="createPresentationRequest()" class="btn employer-button">
              {{ t('tutorial.employer_website.present_digital_diploma_again') }}
            </div>
          </div>
        </div>
      </div>
