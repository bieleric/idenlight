...
<template>
    <div data-type="revokeCredentialContainer">
        ...
        <button @click="revokeCredential(); setRevocationRequested();" class="btn htw-button mt-3 col-12 col-md-3" data-type="revokeCredentialButton">
            <span>{{ t("tutorial.htw_website.revoke_diploma_credential") }}&nbsp;
                <font-awesome-icon v-if="webhookStore.getRevocationStatusRevoked" class="text-white" icon="check" />
                <font-awesome-icon v-if="!webhookStore.getRevocationStatusRevoked && state.revocationRequested" class="text-white" icon="xmark" />
            </span>
        </button>
    </div>
</template>