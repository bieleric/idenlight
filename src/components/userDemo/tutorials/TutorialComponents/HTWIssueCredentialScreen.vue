...
<template>
  <div data-type="issueCredentialContainer">
    ...
    >
      ...
      <div>
        <span v-if="state.showLoadingText" class="font-light font-small text-light">{{
          t('tutorial.htw_website.load_data_of_max_mustermann')
        }}</span>
        <span v-if="state.showCalculationText" class="font-light font-small text-light">{{
          t('tutorial.htw_website.calculate_grade')
        }}</span>
      </div>
    </div>
    ...
    <button
      @click="issueCredential(state.values)"
      class="btn htw-button mt-3 col-12 col-md-3"
      data-type="issueCredentialButton"
    >
      <span
        >{{ t('tutorial.htw_website.issue_digital_diploma') }} &nbsp;<font-awesome-icon
          v-if="webhookStore.getIssuanceStatusOffered"
          class="text-white"
          icon="check"
        />
      </span>
