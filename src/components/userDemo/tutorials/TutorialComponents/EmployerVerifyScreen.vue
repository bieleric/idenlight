...
<template>
    <div class="employer-website-body p-4" data-type="employerWebsiteContainer">
        <div v-if="state.showSpinner" class="spinner-container d-flex flex-column justify-items-center align-items-center">
            ...
            <div>
                <span v-if="state.showVerifyText" class="font-light font-small text-light">{{ t("tutorial.employer_website.check_signature") }}</span>
                <span v-if="state.showLoadingText" class="font-light font-small text-light">{{ t("tutorial.employer_website.load_data") }}</span>
            </div>
        </div>
