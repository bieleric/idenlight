...
<template>
    <div id="navigation-buttons" class="d-flex justify-content-end col-12 mx-auto">
        <div class="px-2">
            <div v-if="userDemoStore.getCurrentStep > 0" @click="decrementStep()" data-type="decrementStep" class="navigation-button btn button-outline-primary">
                <font-awesome-icon icon="chevron-left" />
            </div>
        </div>
        <div>
            <div @click="incrementStep()" data-type="incrementStep" :class="{'pulse-animation': classObject.tutorialFinished, 'navigation-button btn button-primary': true}">
                <font-awesome-icon v-if="userDemoStore.getCurrentStep < userDemoStore.getStepsLength-1" icon="chevron-right" />
                <font-awesome-icon v-if="userDemoStore.getCurrentStep === userDemoStore.getStepsLength-1" icon="xmark" data-type="finishDemoIcon" />
            </div>
        </div>
    </div>
</template>